...
package object scalashop {
  ...
  def boxBlurKernel(src: Img, x: Int, y: Int, radius: Int): RGBA = {
    ...
    def bbox(v: Int, r: Int, max: Int): (Int, Int) = (clamp(v - r, 0, max), clamp(v + r, 0, max))
    val (xFrom, xTo) = bbox(x, radius, src.width - 1)
    val (yFrom, yTo) = bbox(y, radius, src.height - 1)
