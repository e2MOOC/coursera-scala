...
object Anagrams {
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def loop(o: Occurrences): List[Sentence] = {
      if (o.isEmpty) {
        List(Nil)
      } else {
        val combs = combinations(o)
        for (i <- combs if dictionaryByOccurrences.contains(i);
             j <- dictionaryByOccurrences(i);
             s <- loop(subtract(o, i))) yield {j :: s}
      }
    }    
