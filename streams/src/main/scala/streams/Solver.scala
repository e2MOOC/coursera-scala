...
trait Solver extends GameDef {
   ...
   */
  def done(b: Block): Boolean = b.isStanding && (b match {
    case Block(x,y) => goal.x == x && goal.y == y
    case _ => false
  })
  ...
  def from(initial: Stream[(Block, List[Move])],
     ...
     initial match {
       ...
       case (block, moves) #:: tail => {
         var exploredNew = explored + block
         val neighborsNew = newNeighborsOnly(neighborsWithHistory(block, moves), explored + block) 
         neighborsNew ++ from(tail ++ neighborsNew, exploredNew)
       }
     }
  /**
